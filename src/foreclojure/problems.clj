(ns foreclojure.problems
  ...
  (:use     [foreclojure.utils        :only    [from-mongo get-user get-solved login-link *url* flash-msg flash-error def-page row-class approver? can-submit? send-email image-builder with-user as-int]]
...
(defroutes problems-routes
  ...
  (GET "/problem/:id" [id-str]
    (if-let [id (as-int id-str)]
      (problem-page id)
      (flash-error (format "'%s' is not a valid problem number." id-str)
                   "/problems")))
